...
/***************Aahlad**********************************/
	...
	dbg_print("Kernel2:SysMsg: special_file_read begins\n");

	if(file->vn_cdev == bytedev_lookup(file->vn_devid))
	{
		sf_read =file->vn_cdev->cd_ops->read(file->vn_cdev, (int)offset,buf,(int)count);
	}
	dbg_print("Kernel2:SysMsg: special_file_read reads : %d bytes\n",sf_read);
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
	...
	dbg_print("Kernel2:SysMsg: special_file_write begins\n");
        if(file->vn_cdev == bytedev_lookup(file->vn_devid))
        {
                sf_write =file->vn_cdev->cd_ops->write(file->vn_cdev, (int)offset,buf,(int)count);
        }
	dbg_print("Kernel2:SysMsg: special_file_write writes : %d bytes\n",sf_write);
        return sf_write;
